...
public class SnakeLike extends Thread {
    ...
    public void updateGame() {
        ...
        if (fields[playerOne.getGridX()][playerOne.getGridY()].equals(FieldState.B)) {
            // hit!
            endGame();
            return;
        }
        ...
        if (fields[playerOne.getGridX()][playerOne.getGridY()].equals(FieldState.P)) {
            fields[playerOne.getGridX()][playerOne.getGridY()] = FieldState.E;
            playerOne.eat();
            java.util.List<int[]> av = determineAvailableFields(fields);
            Random r = new Random();
            int i = r.nextInt(av.size());
            fields[av.get(i)[0]][av.get(i)[1]] = FieldState.P;
    ...
    public void renderGame(Graphics2D g) {
        ...
        for (int column = 0; column < fields.length; column++) {
            for (int row = 0; row < fields[0].length; row++) {
                if (fields[column][row].equals(FieldState.P)) {
